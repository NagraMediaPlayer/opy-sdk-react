...
function BasicPlayback({ navigation }) {
  ...
  useEffect(() => {
    console.log("Mounting BasicPlayback")
    ...
    return () => {
      ...
      subscription.remove();
      console.log("Unmounting BasicPlayback")
    };
  ...
  const onError = useCallback((event: OnErrorParam) => {
    console.log("onError in BasicPlayback" + JSON.stringify(event));
