...
function Stop({ navigation }) {
  ...
  const rcuKeyHandler = (component, event: { eventType: string }) => {
    console.log("Stop Key event: ", event);
  ...
  const onError = useCallback((event: OnErrorParam) => {
    console.log("onError in Stop" + JSON.stringify(event));
  ...
  const onTextTrackSelected = useCallback((event: OnTextTrackSelectedParam) => {
    ...
    setSelectedText(selectedTextTrackIndex);
  }, []);
  ...
  useEffect(() => {
    ...
    if (!isHandheld() && !isMobileWeb()) {
      console.log("Stop.tsx :: useEffect: set TV Events");
      ...
      <View style={localStyles.viewStyle}>
        <View style={{ flexDirection: "row" }}>
          {Boolean(audioTracks.length) && (
            <View style={{ flex: 1 }}>
              <Text style={localStyles.smallerTextStyle}>Audio</Text>
              <Picker
          ...
          {Boolean(textTracks.length) && (
            <View style={{ flex: 1 }}>
              <Text style={localStyles.smallerTextStyle}>Text</Text>
              <Picker
                selectedValue={
                  selectedText === -1
                    ? "Disabled"
        ...
        <View style={localStyles.buttonRowStyle}>
          ...
          <Button
            ...
            title="Stop"
            onPress={() => rcuKeyHandler(null, { eventType: "Stop" })}
          />
          <Button
