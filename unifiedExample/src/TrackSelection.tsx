...
import { isSafari, isHandheld, isMobileWeb, isLandscape } from "./Utils";

interface RefOTVPlayer {
  current: OTVPlayerRef;
}

const SOURCE_STREAM = isHandheld() || isSafari()
  ? {
    src: "https://d3bqrzf9w11pn3.cloudfront.net/multilanguage_hls_sintel_clear/master.m3u8",
  ...
  const rcuKeyHandler = (component, event: { eventType: string }) => {
    ...
    switch (event.eventType) {
      ...
      case "(un)Mute":
        ...
        break;
      case "audioTrk":
        if (selectedAudioTrackIndex < (mediaTracks && mediaTracks.audioTracks.length - 1)) {
        ...
        break;
      case "textTrk":
        if (selectedTextTrackIndex < (mediaTracks && mediaTracks.textTracks.length - 1)) {
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <View style={localStyles.viewStyle}>
        ...
        <Text style={localStyles.textStyle}>Audio Track: {audioTracks.length > 0 ? audioTracks[selectedAudioTrackIndex].title : "none"}</Text>
        <Text style={localStyles.textStyle}>Text Track: {selectedTextTrackIndex >= 0 ? textTracks[selectedTextTrackIndex].title : "none"}</Text>
